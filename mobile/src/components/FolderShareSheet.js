...
// match the web app (https://vault.example.com/s/<slug>).
const API_BASE =
...
const buildShareUrl = (share, pwd) => {
	...
	const base = `${SITE_ORIGIN}/s/${share.slug}`;
...
/**
 * Public-share manager for a storage folder, rendered inside the shared
 * BottomSheet. Mirrors the web FolderShareModal: pick view/edit, optionally a
 * password and/or expiry, generate a link, then share or revoke any active one.
 * Edit links default the password on (a leaked edit link = full write access).
 */
export default function FolderShareSheet({ visible, onClose, prefix }) {
	...
	const loadShares = async () => {
		if (!prefix) return;
		try {
			setShares(await listFolderShares(prefix));
		} catch {
			// silently ignore
		}
	};
	...
	const handleGenerate = async () => {
		try {
			...
			const created = await createFolderShare({
				prefix,
				accessLevel,
			...
			if (url) {
				try {
					...
					// dismissed — link is still active
				}
			}
		} catch {
			// surfaced via the list staying unchanged
		} finally {
			setLoading(false);
		}
	};
	...
	const handleShare = async (share) => {
		const url = buildShareUrl(share, lastPassword);
		if (!url) return;
		try {
			await Share.share({ message: url });
		} catch {
	...
	return (
		<BottomSheet
			...
			onClose={onClose}
			title="Share folder"
			subtitle={prefix ? `${folderLeaf(prefix)}/` : undefined}
		>
			<ScrollView
				style={styles.body}
			...
			>
				...
				<View style={styles.segment}>
					{[
						...
						<TouchableOpacity
							...
							style={[
								styles.segmentBtn,
								accessLevel === opt.k && styles.segmentBtnActive,
							]}
							onPress={() => setAccessLevel(opt.k)}
						>
							<Text
								style={[
									styles.segmentText,
									accessLevel === opt.k && styles.segmentTextActive,
								]}
							>
								{opt.label}
							</Text>
				...
				<Text style={styles.accessHint}>
					{accessLevel === "edit"
						? "Anyone with the link can upload, delete & create folders."
						: "Anyone with the link can browse & download."}
				</Text>
				...
				<TouchableOpacity
					...
					disabled={loading}
				>
					<Ionicons name="globe-outline" size={16} color={colors.bg} />
				...
				{shares.length > 0 ? (
					<View style={styles.activeWrap}>
						<Text style={styles.activeLabel}>Active links</Text>
						{shares.map((share) => (
...
const styles = StyleSheet.create({
	...
	segmentBtnActive: { backgroundColor: colors.bg, borderColor: colors.accent },
	...
	slug: { flex: 1, color: colors.text, fontFamily: MONO, fontSize: 12 },
	lock: { fontSize: 12 },
	expiry: {
		color: colors.muted,
		fontFamily: MONO,
