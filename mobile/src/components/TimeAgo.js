 ...
 * so the value stays accurate while the user just sits on the screen.
 *
 ...
 * long task list doesn't spin up a timer per row); the timer only runs while at
 * least one <TimeAgo> is mounted.
 *
 * Returns a bare string, so it drops in wherever `timeAgo(value)` was used
 * inside a <Text>:  due <TimeAgo value={t.due_at} />
 */
